...
- **Custos Pendentes (`AdministradorCustosPendentes`)** — lista **toda** OS `Concluída` (`useOrdensServicoTodas({ status: ['Concluída'], busca, lojaId, tipo })`), com ou sem custo já lançado — busca, filtro por loja e `FiltroTipoOS` são resolvidos pelo servidor. Duas abas dentro da própria tela (`FiltroRevisaoCusto`, mesmo par de pílulas com contagem do `FiltroStatusSolicitacao` do Gestor, item 6): **Pendentes** (o que falta o Administrador olhar) e **Revisadas** (o que ele já editou). A OS troca de aba assim que `POST /custo` responde com sucesso — não some da tela, vira histórico.
  ...
  - **Nota fiscal em TODO tipo:** **Número da Nota Fiscal** e **Série** aparecem no modal de qualquer OS, não só de terceiros — maquinário troca peça comprada com nota, reparo consome material com nota, e o Administrador registra o documento que embasa o Custo de Manutenção. Opcionais em todos (nem toda OS teve compra). Foi a **migration `000010`** do back que tirou os dois campos de `ck_custo_por_tipo`; antes disso o servidor recusava NF fora de terceiros com 400. No **card** da listagem a nota só é exibida quando existe — o aviso âmbar "ainda não informada" continua exclusivo de terceiros, onde a falta é pendência de verdade e não ausência de compra.
