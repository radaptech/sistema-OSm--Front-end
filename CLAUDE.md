...
- **Custos Pendentes (`AdministradorCustosPendentes`)** — lista **toda** OS `Concluída` (`useOrdensServicoTodas({ status: ['Concluída'], busca, lojaId, tipo })`), com ou sem custo já lançado — busca, filtro por loja e `FiltroTipoOS` são resolvidos pelo servidor. Duas abas dentro da própria tela (`FiltroRevisaoCusto`, mesmo par de pílulas com contagem do `FiltroStatusSolicitacao` do Gestor, item 6): **Pendentes** (o que falta o Administrador olhar) e **Revisadas** (o que ele já editou). A OS troca de aba assim que `POST /custo` responde com sucesso — não some da tela, vira histórico.
  ...
  - **Nota fiscal: quem decide é o Técnico, no encerramento.** O modal de encerrar (item 11) tem o alternador **"Teve nota fiscal?"** (`temNotaFiscal`, componente `Alternador`) — só quem executou sabe se houve compra (peça, material, fatura da empresa) ou se foi só mão de obra. É essa declaração, e **não o tipo da OS**, que faz **Número da Nota Fiscal** e **Série** aparecerem aqui em Custos Pendentes; desmarcado, o Administrador só confere os valores. O mesmo alternador existe no modal de custos para ele **corrigir** a declaração — sem isso, um Técnico que esqueceu de marcar deixaria a OS sem onde lançar a nota. Desmarcar limpa número e série (`aoSalvarFormulario` zera antes de enviar, espelhando `ck_custo_nota_fiscal`; sem isso o RHF mandaria os valores desmontados e o back devolveria 400). Migrations `000010` (NF deixou de depender do tipo) e `000011` (`tem_nota_fiscal`) no back. No **card** da listagem a linha de nota só aparece quando `custo.temNotaFiscal` — aí o âmbar "ainda não informada" é pendência real, não alarme falso em OS que nunca teve compra.
